  ...
  return <>
    <Modal visible={visible} transparent animationType="slide" onRequestClose={() => { if (!submitting) onClose(); }}>
      <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === "ios" ? "padding" : undefined}><View style={[StyleSheet.absoluteFill, { backgroundColor: theme.overlay }]} /><View accessibilityViewIsModal style={[styles.sheet, { backgroundColor: theme.surfaceElevated }]}>
        <View style={styles.modalHeader}><AppText accessibilityRole="header" style={[styles.modalTitle, { color: theme.text }]}>{t("mobile.addTenant.title")}</AppText><Pressable accessibilityRole="button" accessibilityLabel={t("mobile.addTenant.close")} disabled={submitting} onPress={onClose}><Ionicons name="close" size={26} color={theme.text} /></Pressable></View>
        <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <AppText style={[styles.label, { color: theme.text }]}>{t("mobile.addTenant.room")}</AppText><View style={styles.roomChips}>{vacantRooms.map((room) => { const selected = roomCode === room.roomCode; return <Pressable key={room._id} accessibilityRole="button" accessibilityState={{ selected }} onPress={() => setRoomCode(room.roomCode)} style={[styles.roomChip, { backgroundColor: selected ? theme.primary : theme.background, borderColor: selected ? theme.primary : theme.border }]}><AppText style={[styles.roomChipText, { color: selected ? theme.background : theme.text }]}>{room.roomCode}</AppText></Pressable>; })}</View>
    ...
    <CCCDScannerModal visible={scannerVisible} onClose={() => setScannerVisible(false)} onScan={(cccdNumber, scannedName) => { setIdCard(formatCCCD(cccdNumber)); setLookupIdentifier(cccdNumber); if (scannedName && !fullName.trim()) setFullName(scannedName); }} />
  </>;
}
