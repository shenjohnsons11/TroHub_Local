...
export default function LanguageToggle() {
  ...
  return <View accessibilityRole="radiogroup" accessibilityLabel={t("i18n.language.label")} style={[styles.wrap, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>{options.map((option) => <Pressable key={option.value} accessibilityRole="radio" accessibilityLabel={t(option.value === "vi" ? "i18n.language.switchVietnamese" : "i18n.language.switchEnglish")} accessibilityState={{ selected: language === option.value }} onPress={() => void changeLanguage(option.value)} style={[styles.option, language === option.value && { backgroundColor: theme.primary }]}><AppText style={[styles.text, { color: language === option.value ? theme.background : theme.text }]}>{option.label}</AppText></Pressable>)}</View>;
}
