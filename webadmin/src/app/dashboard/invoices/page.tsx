...
export default function InvoicesPage() {
  ...
  const INVOICE_STEPS = [
    { label: t("invoices.period"), icon: CalendarDays },
  ...
  useEffect(() => {
    if (isAddOpen) {
      ...
      const d = new Date();
      setTitle(`${t("common.month")} ${d.getMonth() + 1}/${d.getFullYear()}`);
    }
  }, [isAddOpen, t]);
  ...
  const handleCreateBulkInvoices = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedItems.length === 0) throw new Error(t("common.error"));
  ...
  const handleRemind = async (invoiceId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      notification.success(t("invoices.reminderSent"));
    } catch (err) {
      notification.error(getNotificationMessage(err, t("common.error")));
    } finally {
      setRemindingId(null);
  ...
  const getStatusBadge = (status: string) => {
    const label = getStatusText("invoice", status, t);
    const normalized = String(status).toLowerCase();
    if (normalized.includes("paid") || normalized.includes("đã thanh toán")) {
      return <Badge className="border-0 bg-primary/10 text-primary">{label}</Badge>;
    }
    if (normalized.includes("overdue") || normalized.includes("quá hạn")) {
      return <Badge className="border-0 bg-destructive/10 text-destructive">{label}</Badge>;
    }
    return <Badge className="border-0 bg-[var(--warning-soft)] text-warning-foreground">{label}</Badge>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader eyebrow={t("nav.overview")} title={t("invoices.title")} description={t("invoices.subtitle")} />
      ...
      <section className="calm-surface flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{t("invoices.createBulk")}</DialogTitle>
            </DialogHeader>
            <ol aria-label={t("invoices.createBulk")} className="mt-3 grid grid-cols-4 gap-2">
            ...
            <form ref={bulkFormRef} onSubmit={handleCreateBulkInvoices} className="mt-5 space-y-5">
              {bulkStep === 1 && <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="title">{t("invoices.period")} *</Label>
                ...
                <Table>
                  <TableHeader className="bg-background">
                    <TableRow>
                      ...
                      <TableHead className="whitespace-nowrap font-semibold">{t("common.room")}</TableHead>
                      <TableHead className="whitespace-nowrap font-semibold">{t("utilities.oldElec")}</TableHead>
                      <TableHead className="whitespace-nowrap font-semibold">{t("utilities.newElec")}</TableHead>
                      <TableHead className="whitespace-nowrap font-semibold">{t("utilities.oldWater")}</TableHead>
                      <TableHead className="whitespace-nowrap font-semibold">{t("utilities.newWater")}</TableHead>
                      <TableHead className="whitespace-nowrap font-semibold text-right">{t("invoices.totalAmount")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {bulkData.length === 0 ? (
                      <TableRow><TableCell colSpan={7} className="text-center py-4">{t("common.noData")}</TableCell></TableRow>
                    ) : (
                          ...
                          <TableRow key={item.contractId} className={!item.selected ? "opacity-50" : ""}>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-1">
                                ...
                                <Button type="button" variant="outline" size="sm" className="h-11 whitespace-nowrap px-2 text-xs" aria-label={t("utilities.scanCamera")} onClick={() => beginMeterCapture(item.contractId, item.room, "electricity")}>📷</Button>
                              </div>
                            </TableCell>
                            <TableCell>
                              ...
                              <Label className="mt-1 block text-[10px] text-muted-foreground" htmlFor={`electricity-price-${item.contractId}`}>{formatCurrency(item.electricityPrice)}</Label>
                              <Input id={`electricity-price-${item.contractId}`} className="mt-1 h-10 w-24 px-2 text-xs bg-card" inputMode="numeric" value={item.electricityPrice} onChange={e => { const u=[...bulkData]; u[index].electricityPrice=utilityPriceOrDefault(unformatNumber(e.target.value), DEFAULT_ELECTRICITY_PRICE); setBulkData(u); }} />
                            ...
                            <TableCell>
                              <div className="flex items-center gap-1">
                                ...
                                <Button type="button" variant="outline" size="sm" className="h-11 whitespace-nowrap px-2 text-xs" aria-label={t("utilities.scanCamera")} onClick={() => beginMeterCapture(item.contractId, item.room, "water")}>📷</Button>
                              </div>
                            </TableCell>
                            <TableCell>
                              ...
                              <Label className="mt-1 block text-[10px] text-muted-foreground" htmlFor={`water-price-${item.contractId}`}>{formatCurrency(item.waterPrice)}</Label>
                              <Input id={`water-price-${item.contractId}`} className="mt-1 h-10 w-24 px-2 text-xs bg-card" inputMode="numeric" value={item.waterPrice} onChange={e => { const u=[...bulkData]; u[index].waterPrice=utilityPriceOrDefault(unformatNumber(e.target.value), DEFAULT_WATER_PRICE); setBulkData(u); }} />
              ...
              {bulkStep === 3 && <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-[20px] bg-primary/10 p-5"><p className="text-sm text-muted-foreground">{t("invoices.period")}</p><p className="mt-1 text-xl font-black">{title}</p></div>
                <div className="rounded-[20px] bg-muted p-5"><p className="text-sm text-muted-foreground">{t("invoices.issuedAt")}</p><p className="mt-1 text-xl font-black">{new Date(issuedAt).toLocaleDateString()}</p></div>
                <div className="rounded-[20px] bg-[var(--warning-soft)] p-5"><p className="text-sm text-muted-foreground">{t("common.confirm")}</p><p className="mt-1 text-3xl font-black">{bulkData.filter((item) => item.selected).length}</p></div>
              </div>}
              {bulkStep === 4 && <div className="calm-surface bg-primary/8 p-6 text-center"><Send className="mx-auto size-9 text-primary" /><h3 className="mt-3 text-xl font-black">{t("common.confirm")}</h3><p className="mt-1 text-muted-foreground">{bulkData.filter((item) => item.selected).length} invoices for {title}</p></div>}
              <div className="flex justify-between gap-3">
                <Button type="button" variant="outline" disabled={bulkStep === 1} onClick={() => setBulkStep((current) => Math.max(1, current - 1))}><ChevronLeft className="size-4" />{t("common.back")}</Button>
                {bulkStep < 4
                  ? <Button type="button" onClick={nextBulkStep}>{t("common.details")}<ChevronRight className="size-4" /></Button>
                  : <Button type="submit" disabled={isSubmitting}><Send className="size-4" />{isSubmitting ? t("common.loading") : `${t("common.send")} (${bulkData.filter((item) => item.selected).length})`}</Button>}
              </div>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="calm-workbench">
        <Table>
          <TableHeader className="bg-background">
            <TableRow>
              <TableHead className="font-semibold text-foreground">{t("invoices.invoiceCode")}</TableHead>
              <TableHead className="font-semibold text-foreground">{t("invoices.period")}</TableHead>
              <TableHead className="font-semibold text-foreground">{t("common.room")}</TableHead>
              <TableHead className="font-semibold text-foreground">{t("invoices.totalAmount")}</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="h-64 text-center"><Image src="/trohub-empty-states.png" alt="" width={170} height={100} className="mx-auto h-24 w-40 rounded-[20px] object-cover object-left" /><p className="mt-3 font-black">{t("invoices.emptyInvoices")}</p></TableCell>
              </TableRow>
            ) : (
              filteredInvoices.map(invoice => (
                <TableRow key={invoice._id || invoice.id} className="cursor-pointer hover:bg-accent/40" onClick={() => setDetailInvoice(invoice)}>
                  ...
                  <TableCell><span className="font-semibold">{invoice.roomCode || "-"}</span><span className="block text-xs text-muted-foreground">{invoice.nguoiThue || "-"}</span></TableCell>
                  <TableCell>{formatCurrency(invoice.totalAmount)}</TableCell>
                  <TableCell>{getStatusBadge(invoice.statusLabel || invoice.status)}</TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1" onClick={e => e.stopPropagation()}>
                      <Button aria-label={t("common.details")} onClick={e => { e.stopPropagation(); setDetailInvoice(invoice); }} variant="ghost" size="icon" title={t("common.details")} className="text-muted-foreground hover:text-foreground">
                        <Eye className="size-4" />
                      </Button>
                      {String(invoice.statusLabel).toLowerCase() !== "đã thanh toán" && String(invoice.statusLabel).toLowerCase() !== "paid" && (
                        <>
                          <Button
                          ...
                          <Button aria-label={t("invoices.markPaid")} onClick={e => { e.stopPropagation(); handleMarkPaid(invoice._id || invoice.id); }} variant="ghost" size="icon" title={t("invoices.markPaid")} className="text-primary hover:bg-primary/10 hover:text-primary">
                            <CheckCircle className="size-4" />
                          </Button>
                        </>
                      )}
                      <Button aria-label={t("common.delete")} onClick={e => { e.stopPropagation(); handleDelete(invoice._id || invoice.id); }} variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 hover:text-destructive">
