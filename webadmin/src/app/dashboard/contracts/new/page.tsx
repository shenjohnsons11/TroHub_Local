...
export default function NewContractPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><header><p className="text-sm font-bold uppercase tracking-[.12em] text-primary">HỢP ĐỒNG mới</p><h1 className="mt-1 text-3xl font-black">Tạo hợp đồng thuê</h1><p className="mt-2 text-muted-foreground">Bản nháp được tự động lưu riêng cho tài khoản Admin.</p></header><ol className="grid grid-cols-4 overflow-hidden rounded-[14px] border border-border bg-card">{CONTRACT_STEPS.map((item) => <li key={item.id} className={`relative p-4 text-sm ${item.id === step ? "bg-primary text-primary-foreground" : item.id < step ? "bg-primary/10 text-primary" : "text-muted-foreground"}`}><span className="flex items-center gap-2 font-bold">{item.id < step ? <Check className="h-4 w-4" /> : <span>{item.id}</span>}<span className="hidden sm:inline">{item.label}</span></span></li>)}</ol><section className="min-h-[420px] rounded-[16px] border border-border bg-card p-6 sm:p-8">
    {step === 1 && <div className="grid gap-5 md:grid-cols-2"><Field label="Phòng" error={errors.roomId}><select className="h-11 w-full rounded-md border border-input bg-background px-3" value={draft.roomId} onChange={(e) => { update("roomId", e.target.value); const room = rooms.find((item) => (item._id || item.id) === e.target.value); if (room) setDraft((value) => ({ ...value, roomId: e.target.value, fixedRentPrice: String(room.defaultRentPrice || ""), fixedDeposit: String(room.defaultDeposit || room.defaultRentPrice || "") })); }}><option value="">Chọn Phòng</option>{rooms.map((room) => <option key={room._id || room.id} value={room._id || room.id}>{room.roomCode}</option>)}</select></Field><Field label="Người thuê" error={errors.tenantId}><select className="h-11 w-full rounded-md border border-input bg-background px-3" value={draft.tenantId} onChange={(e) => update("tenantId", e.target.value)}><option value="">Chọn Người thuê</option>{nguoiThueList.map((item) => <option key={item._id || item.id} value={item._id || item.id}>{item.fullName || item.name} · {item.phone}</option>)}</select></Field></div>}
  ...
  </section><footer className="flex justify-between"><Button variant="outline" disabled={step === 1} onClick={() => setStep((value) => Math.max(1, value - 1))}><ChevronLeft className="mr-2 h-4 w-4" />Quay lại</Button>{step < 4 ? <Button onClick={next}>Tiếp tục<ChevronRight className="ml-2 h-4 w-4" /></Button> : <Button disabled={submitting} onClick={() => void submit()}>{submitting ? "Đang tạo..." : "Tạo hợp đồng"}</Button>}</footer></div>;
}
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute right-1 top-1 flex h-9 w-9 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        onClick={openPicker}
      >
